...
for group in groups:
    ...
    # A. The Histogram (Background Context)
    # Increased bins for smoother look, lighter color
    ...
    max_hist_height = max(n) if len(n) > 0 else 0
    ...
    # C. Today's Points (The Fix: Scatter instead of Lines)
    if not df_today.empty:
        outlier_limit = mu + (Z_SCORE_THRESHOLD * sigma)
        
        # Split Today's data into Normal vs Outliers
        today_outliers = df_today[df_today['Time_sec'] > outlier_limit]
        today_normal = df_today[df_today['Time_sec'] <= outlier_limit]
        
        # Define a Y-height for the dots (approx 5% of max height) so they sit on the bottom
        dot_y = max_hist_height * 0.05 
        
        # 1. Plot Normal Nodes (Green Circles, Clean, No Labels)
        ...
        # 2. Plot Outliers (Red X's, Larger, Labeled)
        if not today_outliers.empty:
            plt.scatter(today_outliers['Time_sec'], [dot_y] * len(today_outliers), 
                        ...
                        label="Today: Outliers (>2$\\sigma$)", zorder=4)

            # Annotation Loop (Only for Outliers)
            for _, row in today_outliers.iterrows():
                val = row['Time_sec']
                node = row['Node']
                z_score = (val - mu) / sigma
                
                # Place text slightly above the dot with an arrow pointing down
                plt.annotate(f"{node}\n(+{z_score:.1f}$\\sigma$)", 
                             xy=(val, dot_y), 
                             xytext=(val, dot_y + (max_hist_height * 0.15)),
    ...
    plt.grid(axis='y', alpha=0.2) # Horizontal grid only
    plt.legend(loc='upper right', frameon=True)
    
    # Add a little padding to the top so labels don't get cut off
    plt.ylim(0, max(max(p), max_hist_height) * 1.2)

    filename = f'distribution_{group}.png'
    save_path = os.path.join(OUTPUT_DIR, filename)
    plt.savefig(save_path, dpi=100) # Added DPI for sharper image
    plt.close()
